...
<div class="feed">
  ...
  {:else}
    <Card variant="white" class="sign-in-prompt">
      <h2 class="text-display">Welcome!</h2>
      <p>You're viewing the public feed. <a href="/">Sign in</a> to post and interact.</p>
    </Card>
  {/if}
  ...
  <div class="posts">
    ...
    {:else}
      {#each data.posts as post (post.id)}
        <PostCard {post} currentUserId={data.user?.id} isAdmin={data.user?.isAdmin || false} showReplyButton={!!data.user} />
      {/each}
    {/if}
  </div>
    ...
    <Card variant="lime" class="pagination">
      <div class="pagination-controls">
        {#if data.pagination.page > 1}
          <a href="/feed?page={data.pagination.page - 1}" class="page-button bordered bordered-hover">
            ← Previous
          </a>
        {:else}
...
<style>
  ...
  :global(.sign-in-prompt) h2 {
    font-size: calc(var(--unit) * 1);
    margin-bottom: calc(var(--unit) * 0.3);
  }

  :global(.sign-in-prompt) p {
    font-size: calc(var(--unit) * 0.75);
    margin: 0;
  }

  :global(.sign-in-prompt) a {
    color: black;
    font-weight: 900;
    text-decoration: underline;
  }

  :global(.sign-in-prompt) a:hover {
    color: white;
  }
